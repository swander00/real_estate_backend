...
async function fetchAndProcessBatch(tableConfig, state) {
  ...
  try {
    ...
    if (enforceParent) {
      // For all tables, use ListingKey
      const keys = Array.from(new Set(mapped.map(r => r && r.ListingKey).filter(Boolean)));
      
      if (DEBUG) {
        dlog(`    🔍 ${name} parent enforcement: checking ${keys.length} unique ListingKeys`);
        if (keys.length > 0) {
          dlog(`    📋 Sample ListingKeys: ${keys.slice(0, 5).join(', ')}${keys.length > 5 ? '...' : ''}`);
        }
      }
      ...
      mapped = mapped.filter(r => r && existing.has(r.ListingKey));
      ...
      if (DEBUG && orphanDropped > 0) {
        ...
        // Log sample orphaned ListingKeys for debugging
        const orphanedKeys = mapped.map(r => r && r.ListingKey).filter(key => key && !existing.has(key));
        const uniqueOrphaned = Array.from(new Set(orphanedKeys)).slice(0, 10);
        if (uniqueOrphaned.length > 0) {
          dlog(`    🚫 Sample orphaned ListingKeys: ${uniqueOrphaned.join(', ')}${uniqueOrphaned.length === 10 ? '...' : ''}`);
        }
        
        // Stage orphaned records for later processing instead of dropping them
        const orphanedRecords = mapped.filter(r => r && r.ListingKey && !existing.has(r.ListingKey));
