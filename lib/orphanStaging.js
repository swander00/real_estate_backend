  ...
  try {
    ...
    if (DEBUG) {
      ...
      sampleRecords.forEach((record, idx) => {
        console.log(`  Sample ${idx + 1}: ListingKey=${record.ListingKey}, ${tableName === 'property_openhouse' ? 'OpenHouseKey' : tableName === 'property_rooms' ? 'RoomKey' : 'MediaKey'}=${record[tableName === 'property_openhouse' ? 'OpenHouseKey' : tableName === 'property_rooms' ? 'RoomKey' : 'MediaKey']}`);
      });
    }
...
export async function processStagedOrphanedRecords(tableName) {
  try {
    ...
    // Get unique ListingKeys from staged records
    const listingKeys = [...new Set(stagedRecords.map(r => r.ListingKey).filter(Boolean))];

    // Check which ListingKeys now exist in common_fields
    ...
    const processableRecords = stagedRecords.filter(r => 
      r.ListingKey && existingParentKeys.has(r.ListingKey)
    );
