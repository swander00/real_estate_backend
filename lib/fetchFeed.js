  ...
  if (next) {
    ...
    if (filter) {
      if (baseUrl.includes('$filter=')) {
        ...
        // Extract existing filter and combine with new filter using 'and'
        const existingFilterMatch = baseUrl.match(/\$filter=([^&]+)/);
        if (existingFilterMatch) {
          const existingFilter = decodeURIComponent(existingFilterMatch[1]);
