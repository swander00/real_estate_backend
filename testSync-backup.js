...
const combineUnique = (a, b) => Array.from(
  new Map([...a, ...b].map(i => [i.ListingKey, i])).values()
);

async function testSync() {
  console.log('🧼 Clearing all tables');
  const tables = ['common_fields','residential_freehold','residential_condo','residential_lease','property_openhouse','property_media'];
  await Promise.all(
  ...
  const openhouseRaw = (await fetchAndLog('OpenHouse')).filter(o => o.OpenHouseKey && o.ListingKey);
  const mediaRaw = (await fetchAndLog('Media')).filter(m => m.ListingKey && m.MediaURL);
  ...
  const commonRows = Array.from(commonMap.entries()).map(([key, { idx, vow }]) => ({
    ListingKey: key,
    ...mapCommonFields(idx, vow)
  }));
 ...
 // Map raw media items into rows matching the new composite PK on (ListingKey, MediaKey)
...
await upsert('property_media', mediaRows, ['ListingKey', 'MediaKey'], r => r.ListingKey && r.MediaKey);
